...
| Item | Status | Note |
|---|---|---|
| Privacy Policy | PENDING | `docs/PRIVACY_POLICY.md` (new) is written to match the in-app screen (`lib/features/settings/privacy_screen.dart`) exactly — Play Console requires a **publicly hosted** URL, not just in-app text or a repo file. Publish `PRIVACY_POLICY.md`'s content at a real URL before submission. |
